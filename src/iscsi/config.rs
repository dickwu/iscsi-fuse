...
// ---------------------------------------------------------------------------
// CliArgs — clap-derived command-line arguments
// ---------------------------------------------------------------------------

/// Command-line arguments for iscsi-fuse.
#[derive(Parser, Debug, Clone)]
#[command(name = "iscsi-fuse")]
#[command(about = "Mount an iSCSI target as a FUSE filesystem on macOS")]
pub struct CliArgs {
    /// Path to TOML config file [default: ~/.iscsi-fuse.toml]
    #[arg(short = 'c', long)]
    pub config: Option<PathBuf>,

    /// FUSE mount-point directory [default: /Volumes/<volume-name>]
    #[arg(short = 'm', long)]
    ...
    /// Override the LUN number from the config file.
    #[arg(short = 'l', long)]
    pub lun: Option<u64>,
...
impl CliArgs {
    /// Resolve the config-file path, falling back to `~/.iscsi-fuse.toml`.
    pub fn resolved_config(&self) -> PathBuf {
        self.config.clone().unwrap_or_else(|| {
            dirs::home_dir()
                .unwrap_or_else(|| PathBuf::from("."))
                .join(".iscsi-fuse.toml")
        })
    }

    /// Resolve the mount point, falling back to `/Volumes/<volume_name>`.
